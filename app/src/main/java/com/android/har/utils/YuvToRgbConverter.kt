...
import com.android.har.BuildConfig
import java.nio.ByteBuffer

class YuvToRgbConverter(context: Context) {
    private val renderScript = RenderScript.create(context)
    private val scriptYuvToRgb = ScriptIntrinsicYuvToRGB.create(renderScript, Element.U8_4(renderScript))
    ...
    fun yuvToRgb(image: Image, output: Bitmap) {
        ...
        if (!::inputAllocation.isInitialized) {
            val elemType = Type.Builder(renderScript, Element.YUV(renderScript)).setYuvFormat(ImageFormat.NV21).create()
            inputAllocation = Allocation.createSized(renderScript, elemType.element, yuvBuffer.array().size)
        }
